...
const ArtistPortfolio = () => {
  ...
  const displayUploadPictureButton = () => {
    if (artist !== null && artist.id === match.params.id) {
      return (
        <button type="button" class="btn btn-primary uploadPictureArtistButton" data-toggle="modal" data-target="#uploadPictureModalCenter" onClick={handleClick}>
          +Upload picture
        </button>
      );
    }
  ...
  const displayUploadVideoButton = () => {
    if (artist !== null && artist.id === match.params.id) {
      return (
        <button type="button" class="btn btn-primary uploadVideoArtistButton" data-toggle="modal" data-target="#uploadVideoModalCenter" onClick={handleClick}>
          +Upload video
        </button>
      );
    }
